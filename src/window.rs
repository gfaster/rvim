...
impl DispComponent for LineNumbers {
    fn draw(&self, win: &Window, _ctx: &Ctx) {
        for l in 0..win.height() {
            ...
            term::goto(TermPos { x: winbase.x - 4, y: winbase.y });
...
impl DispComponent for RelLineNumbers {
    fn draw(&self, win: &Window, _ctx: &Ctx) {
        for l in 0..win.height() {
            ...
            term::goto(TermPos { x: winbase.x - 5, y: winbase.y });
...
impl Window {
    pub fn new(buf: Buffer) -> Self {
        ...
        let padding = components.iter().fold(Padding { top: 0, bottom: 0, left: 0, right: 0 }, |acc, x| {
            let pad = x.padding();
            Padding {
                top: acc.top + pad.top,
        ...
        Self {
            ...
            topleft: TermPos { x: 1 + padding.left, y: 1 + padding.top },
            botright: TermPos { x: 90 - padding.right, y: 40 - padding.bottom },
            components
        }
    }
...
mod test {
    ...
    fn basic_window() -> Window {
        ...
        Window { buf: b, topline: 0, cursorpos: TermPos { x: 0, y: 0 }, cursoroff: 0, topleft: TermPos { x: 0, y: 0 }, botright: TermPos { x: 7, y: 32 }, components: vec![]}
    }
